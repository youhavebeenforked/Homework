...
public class CalculatorTest {
    ...
    private static DecimalFormat df4 = new DecimalFormat(".####"); //‘ормат вывода даблов
    public CalculatorTest(){
        //ѕо умолчанию разделитель дробной части зап€та€, а нужна точка
    ...
    //ѕроверка умножени€ интов
    void multiplyIntTest(double a, double b) {
        int first= (int) a;
        int second= (int) b;

        notValidArgumentTest(a, b, 3); //€вл€ютс€ ли введенные значени€ целыми числами
        overflowIntVariableTest(a, b); //¬лаз€т ли эти цисла в инт
        overflowTest(first,second,3); //переполн€етс€ ли результат инт после умножени€

        int res = calc.multiply(first, second);
        Assert.equals(first + "*" + second + " must be equals" + first * second + ", not a " + res, res, first * second);
    }

    //ѕроверка вычитани€ в даблах
    void subDoubleTest(double a, double b, double actual){
        overflowTest(a,b,6); //ѕроверка переполнени€ дабла
    ...
    void divisionDoubleTest(double a, double b, double actual){
        divisionByZeroTest(a, b, 8); // ѕроверка делени€ на ноль
    ...
    public void run(){
        ...
        //переполнение инта после вычислени€
        try {
